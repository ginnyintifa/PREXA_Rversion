...
## 4 Map the variance to the PTM sites, and get the counts


## only consider mappable mutations here
...
treat_each_row=function(proteinID, ptm_pos, whole_snp)
{
    a_total_mut=0
    a_missense_mut=0
    a_nonsense_mut=0
    ...
    if (length(wanted_snp_row)!=0)
    {
        ...
        near_mut_start=which(as.numeric(as.character(snp_wanted$aaposstart))<=(ptm_pos+5)&as.numeric(as.character(snp_wanted$aaposstart))>=(ptm_pos-5))
        near_mut_end=which(as.numeric(as.character(snp_wanted$aaposend))<=(ptm_pos+5)&as.numeric(as.character(snp_wanted$aaposend))>=(ptm_pos-5))
...
get_ptm_mapping = function (stomach_all, output_name)
{
    ...
    cancer_snp=sapply(1:nrow(ptm_data),
    function(i){
        protid=ptm_data$ptm_ID[i]
        ptmpos=as.numeric(as.character(ptm_data$ptm_position[i]))
        
        this_mut=treat_each_row(protid, ptmpos, new_snp_df)
...
#ptm_data=fread("/Users/ginny/tcga_june_gastric_cancer/ptm_data_aggregate.tsv", header = T, stringsAsFactors = F)

##may consider putting this to the function parameters

# for stomach
#etwd ("/Users/ginny/tcga_june_gastric_cancer/TCGA_STAD")
